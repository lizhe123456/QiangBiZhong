...
import com.whmnrc.qiangbizhong.ui.shop.shopenter.location.utils.DatasKey;
import com.whmnrc.qiangbizhong.ui.shop.shopenter.location.utils.OnItemClickLisenter;
import com.whmnrc.qiangbizhong.ui.shop.shopenter.location.utils.SPUtils;
...
public class SelectAddressActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_select_address1);
        initView();
                ...
                try {
                    if (null != loc) {
                        ...
                        if (loc.getErrorCode() == 0) {//可在其中解析amapLocation获取相应内容。
                            location = loc;
                            SPUtils.putString(SelectAddressActivity.this, DatasKey.LOCATION_INFO, gson.toJson(location));
...
//                        Toast.makeText(MainActivity.this, "搜索", Toast.LENGTH_SHORT).show();
                        startActivityForResult(new Intent(SelectAddressActivity.this, SearchActivity.class), SEARCHREQUESTCODE);
                        break;
...
//                        Toast.makeText(MainActivity.this, "定位", Toast.LENGTH_SHORT).show();
                    ...
                    case R.id.bt_send:
                        if (null != mList && 0 < mList.size() && null != mAddressAdapter) {
                            ...
                            setResult(101,new Intent().putExtra("city", poiItem.getProvinceName() + poiItem.getCityName()+ poiItem.getAdName()).putExtra("latitude",poiItem.getLatLonPoint().getLatitude()+"").putExtra("longitude",poiItem.getLatLonPoint().getLongitude()+""));
                            finish();
//                            Toast.makeText(SelectAddressActivity.this, "发送：" + poiItem.getTitle() + "  " + poiItem.getSnippet() + "  " + "纬度：" + poiItem.getLatLonPoint().getLatitude() + "  " + "经度：" + poiItem.getLatLonPoint().getLongitude(), Toast.LENGTH_SHORT).show();
                        }
                        break;
                }
            }
